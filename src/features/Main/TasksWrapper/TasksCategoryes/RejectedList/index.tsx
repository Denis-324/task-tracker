...
import { RejectedListEffects, RejectedListSelectors, StatusesTypes, Types } from 'store';
...
export const RejectedList = () => {
  ...
  } = useSelector(RejectedListSelectors.pagination);

  const ref = useRef<HTMLDivElement>(null);

  const listData = useSelector(RejectedListSelectors.data);
  const status = useSelector(RejectedListSelectors.status);
  const sort = useSelector(RejectedListSelectors.sort);

  const dispatch = useDispatch();
  const changePageSize = (size: number) => {
    dispatch(RejectedListEffects.fetchAll({ per_page: size, page: 1 }));
    ...
    <BoxWrap ref={ref}>
      <InboxWrap>
        <List
          header={
            <HeaderCard
              title={Types.TitlesCards.rejected}
              taskStatus={StatusesTypes.statusesId.rejected}
              listSort={sort}
            />
          }
