...
import { CompletedListEffects, CompletedListSelectors, StatusesTypes, Types } from 'store';
...
export const CompletedList = () => {
  ...
  } = useSelector(CompletedListSelectors.pagination);

  const ref = useRef<HTMLDivElement>(null);

  const listData = useSelector(CompletedListSelectors.data);
  const status = useSelector(CompletedListSelectors.status);
  const sort = useSelector(CompletedListSelectors.sort);
  const dispatch = useDispatch();

  const changePageSize = (size: number) => {
    dispatch(CompletedListEffects.fetchAll({ per_page: size, page: 1 }));
    ...
    <BoxWrap ref={ref}>
      <InboxWrap>
        <List
          header={
            <HeaderCard
              title={Types.TitlesCards.completed}
              taskStatus={StatusesTypes.statusesId.completed}
              listSort={sort}
            />
          }
