...
import { NotCompletedListEffects, NotCompletedListSelectors, StatusesTypes, Types } from 'store';
...
export const NotCompletedList = () => {
  const {
    per_page: pageSize,
    items_total: itemsTotal,
    page_current: pageCurrent,
  } = useSelector(NotCompletedListSelectors.pagination);

  const ref = useRef<HTMLDivElement>(null);

  const listData = useSelector(NotCompletedListSelectors.data);
  const status = useSelector(NotCompletedListSelectors.status);
  const sort = useSelector(NotCompletedListSelectors.sort);

  const dispatch = useDispatch();

  const changePageSize = (size: number) => {
    dispatch(NotCompletedListEffects.fetchAll({ per_page: size, page: 1 }));
    ...
    <BoxWrap ref={ref}>
      <InboxWrap>
        <List
          header={
            <HeaderCard
              title={Types.TitlesCards.notCompleted}
              taskStatus={StatusesTypes.statusesId.notCompleted}
              listSort={sort}
            />
          }
