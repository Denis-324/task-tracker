...
export const Status: React.FC = () => {
    ...
    if (!incomingItems) {
      dispatch(
        FilterActions.setStatus(
          filterStatuses.filter((status) => status !== StatusesTypes.statusesId.created),
        ),
      );
    }
  }, [incomingItems]);
  ...
  useEffect(() => {
    if (!atWorkItems) {
      dispatch(
        FilterActions.setStatus(
          filterStatuses.filter((status) => status !== StatusesTypes.statusesId.inWork),
        ),
      );
    }
  }, [atWorkItems]);
  ...
  useEffect(() => {
    if (!completedItems) {
      dispatch(
        FilterActions.setStatus(
          filterStatuses.filter((status) => status !== StatusesTypes.statusesId.completed),
        ),
      );
    }
  }, [completedItems]);
  ...
  useEffect(() => {
    if (!notCompletedItems) {
      dispatch(
        FilterActions.setStatus(
          filterStatuses.filter((status) => status !== StatusesTypes.statusesId.notCompleted),
        ),
      );
    }
  }, [notCompletedItems]);
  ...
  useEffect(() => {
    if (!rejectedItems) {
      dispatch(
        FilterActions.setStatus(
          filterStatuses.filter((status) => status !== StatusesTypes.statusesId.rejected),
        ),
      );
    }
  }, [rejectedItems]);
  ...
  return (
    <Wrap>
      <p>СТАТУС</p>
      <Checkbox.Group value={filterStatuses} onChange={onChangeHandler}>
        {statuses.map((status) => (
